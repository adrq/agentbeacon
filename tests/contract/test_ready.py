...
class TestSchedulerReadyEndpoint:
    ...
    def _make_ready_request(self, port, timeout=5):
        ...
        try:
            response = requests.get(f"http://localhost:{port}/api/ready", timeout=timeout)
            return response
    ...
    def test_ready_endpoint_eventually_returns_200(self):
        """Test that /api/ready eventually returns 200 when scheduler is ready."""
        # NOTE: This test will fail until the /api/ready endpoint is implemented
        ...
        try:
            ...
            assert server_started, "Scheduler did not start accepting connections within 10 seconds"
            ...
            while time.time() - start_time < 15:  # Wait up to 15 seconds for readiness
                ...
                if response is not None:
                    ...
                    if response.status_code == 200:
                        ...
                        pytest.fail(f"Unexpected status code {response.status_code}, expected 200 or 503")
            ...
            # Common ready response formats
            valid_ready_statuses = ["ready", "ok", "operational"]
            assert response_json["status"] in valid_ready_statuses, \
                f"Expected status to be one of {valid_ready_statuses}, got '{response_json['status']}'"
    ...
    def test_ready_endpoint_accepts_only_get_method(self):
    ...
    def test_ready_endpoint_consistency_after_ready(self):
        ...
        try:
            ...
            for i in range(5):
                response = requests.get(f"http://localhost:{self.test_port}/api/ready", timeout=5)
    ...
    def test_ready_endpoint_503_before_ready_state(self):
