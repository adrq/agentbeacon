...
def test_worker_complete_task_execution_cycle():
    ...
    try:
        ...
        assert orch_ready, f"Mock orchestrator did not start on port {mock_orchestrator_port}"
        ...
        response = requests.post(f"http://localhost:{mock_orchestrator_port}/add_task", json=sample_task)
        ...
        time.sleep(8)  # Give worker time to poll, execute, and report
        ...
        assert "task-123" in worker_output, f"Worker should process task: {worker_output}"
        assert "completed successfully" in worker_output, f"Task should complete: {worker_output}"
        assert "Result posted successfully" in worker_output, f"Result should be posted: {worker_output}"

        # Verify worker continues polling after task completion (multiple polling cycles)
        polling_lines = [line for line in worker_output.split('\n') if 'Starting worker loop' in line or 'No task available' in line]
        assert len(polling_lines) >= 1, f"Worker should show polling activity: {worker_output}"
...
def test_worker_handles_task_with_output():
    ...
    try:
        ...
        output_task = {
            ...
            "agent": "mock-agent",
            "request": {
                "input": "Generate a test response with some output"
            }
        }

        # Add task to orchestrator
        response = requests.post(f"http://localhost:{mock_orchestrator_port}/add_task", json=output_task)
        assert response.status_code == 200
        ...
        time.sleep(8)  # Give worker time to process task
        ...
        assert "output-task-456" in worker_output, f"Worker should process task: {worker_output}"
        assert "completed successfully" in worker_output, f"Task should complete: {worker_output}"
        ...
        if response.status_code == 200:
            ...
            assert len(results) >= 1, f"Worker should post result: {results}"
            result = results[0]
            assert "output-task-456" in str(result), f"Result should contain task ID: {result}"
            # Result should contain the mock-agent output
            assert "nodeId" in result and "status" in result, f"Result should have required fields: {result}"
...
def test_multiple_task_execution_sequence():
    ...
    try:
        ...
        time.sleep(12)  # Give worker time to process both tasks
        ...
        assert "seq-task-1" in worker_output, f"Worker should process first task: {worker_output}"
        assert "seq-task-2" in worker_output, f"Worker should process second task: {worker_output}"

        # Count successful completions
        completed_count = worker_output.count("completed successfully")
        assert completed_count >= 2, f"Expected 2 completed tasks, got {completed_count}: {worker_output}"

        # Check that results were posted back to orchestrator
        response = requests.get(f"http://localhost:{mock_orchestrator_port}/results")
        if response.status_code == 200:
            results = response.json()
            assert len(results) >= 2, f"Worker should post results for both tasks: {results}"

            # Check that both task IDs are present in results
            results_str = str(results)
            assert "seq-task-1" in results_str, f"Results should contain first task: {results}"
            assert "seq-task-2" in results_str, f"Results should contain second task: {results}"
